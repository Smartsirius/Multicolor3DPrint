    ...
    public partial class MainForm : Form
    {
        ...
        private void ToolStripButtonGenerateCode_Click(object sender, EventArgs e)
        {
            ...
            foreach (var extruder in allExtruder)
            {
                ...
                if (currentTool != null)
                {
                    var red = Math.Round(myPrinter.PrintColor.ProportionOfRed * 10.0) / 10.0;
                    var green = Math.Round(myPrinter.PrintColor.ProportionOfGreen * 10.0) / 10.0;
                    // var blue = Math.Round(myPrinter.PrintColor.ProportionOfBlue * 10.0) / 10.0;
                    var blue = 1 - (red + green);

                    text += string.Format("; Color: {0}\n", currentTool.Color.Name);
                    text += string.Format("M163 S{0} P{1}\n", redExtr.Number, red.ToString("0.0", languageForNumberFormat.NumberFormat)); //.ToString("0.000", languageForNumberFormat.NumberFormat));
                    text += string.Format("M163 S{0} P{1}\n", greenExtr.Number, green.ToString("0.0", languageForNumberFormat.NumberFormat)); //.ToString("0.000", languageForNumberFormat.NumberFormat));
                    text += string.Format("M163 S{0} P{1}\n", blueExtr.Number, blue.ToString("0.0", languageForNumberFormat.NumberFormat)); //.ToString("0.000", languageForNumberFormat.NumberFormat));
